...
- **Inputs:**
  ...
  - Logto Management API responses for the current user's personal access tokens, including token `name`, `value`, `createdAt`, and `expiresAt`; Logto-owned metadata fields such as tenant or user identifiers are accepted but ignored by the browser contract.
...
- **Key Elements:**
  - **Authentication gate:** The Dashboard route is protected by the shared web auth coordinator. Dashboard data endpoints require an authenticated web session. Anonymous or expired-session requests receive a safe auth error and do not create, list, rename, or delete tokens.
  ...
  - **Browser error contract:** Dashboard endpoints use the shared web auth error codes `authentication_required` and `session_expired` for missing or expired sessions, `400 dashboard_invalid_token_name` for invalid names, `404 dashboard_token_not_found` for missing tokens, `409 dashboard_token_name_conflict` for duplicate names, and a safe dashboard dependency error when Logto token lifecycle calls cannot be completed. Logto response metadata fields must not cause dependency failures when the required token fields are present. MCP usage-summary failure does not fail the token directory; the BFF returns tokens with `usageAvailable: false`, `usageCount: null`, and `lastUsedAt: null`.
  ...
  - **Copy flow:** The row copy control copies the token value currently returned by the BFF for that authenticated owner. After the browser clipboard write succeeds, the clicked token's copy affordance switches from the lucide copy icon to a lucide check icon for `3` seconds, then returns to the copy icon. Repeated successful copies reset the `3` second timer. Copy success is reflected through local UI state without sending token values back to the server.
  ...
  - **Visual language:** Dashboard styling follows the approved Knowledge Figma variables: light page background, translucent/card white surfaces, subtle blue-gray borders, `8px` radii, Geist typography, restrained spacing, and lucide icons for copy/menu/close/actions. The Dashboard page header uses the shared routed-page header tokens defined by the app shell: `layout/page/header-height`, `typography/page/title/font-size`, and `typography/page/title/line-height`. Repeated dashboard colors, radii, shadows, spacing, typography, and button treatments are represented as Tailwind `@theme` tokens rather than per-component hard-coded utility values.
...
- **Checks:**
  ...
  - BFF Logto adapter tests verify Logto metadata fields are tolerated when required PAT fields are present.
  ...
  - Frontend tests cover dashboard loading, quota panel rendering, inactive quota windows, empty state, token table rendering, mobile row rendering, create dialog, rename dialog, delete confirmation, copy control behavior, tokenized button/dialog styling, authenticated error handling, and account-menu navigation.
  - Visual inspection confirms the Dashboard page header uses the shared routed-page header height and title typography tokens rather than dashboard-local title sizing.
