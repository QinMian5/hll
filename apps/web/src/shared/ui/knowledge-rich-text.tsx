...
export interface KnowledgeRichTextProps {
  ...
  readonly variant: "content" | "leaf-label" | "title";
}
...
const fallbackClasses = {
  ...
  "leaf-label":
    "min-w-0 text-[11px] leading-[1.25] font-medium text-[rgba(38,52,77,0.82)] whitespace-pre-wrap",
  title:
...
const leafLabelContainerClasses =
  "min-w-0 text-center text-[11px] leading-[1.25] font-medium text-[rgba(38,52,77,0.82)] [&_.katex]:text-[rgba(38,52,77,0.82)] [&_.katex-display]:my-0 [&_.katex-display]:overflow-x-hidden [&_.katex-display]:overflow-y-hidden [&_code]:rounded-[4px] [&_code]:bg-transparent [&_code]:p-0 [&_ol]:m-0 [&_ol]:list-none [&_ol]:p-0 [&_p]:m-0 [&_pre]:m-0 [&_ul]:m-0 [&_ul]:list-none [&_ul]:p-0";
...
export const KnowledgeRichText = memo(function KnowledgeRichText({
  ...
  const className =
    variant === "title"
      ...
      : variant === "leaf-label"
        ? leafLabelContainerClasses
        : contentContainerClasses;
  const markdownComponents = useMemo(
    () => ({
      p: ({ children }: { readonly children?: ReactNode }) =>
        variant === "title" || variant === "leaf-label" ? (
          <span>{children}</span>
        ) : (
          <p>{children}</p>
        ),
