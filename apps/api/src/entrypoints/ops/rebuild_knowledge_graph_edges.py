...
async def run_rebuild(
    *,
    apply: bool,
    edge_similarity_top_k: int | None,
    edge_similarity_min_strength: float | None,
) -> EdgeRebuildResult:
    runtime = get_runtime_dependencies()
    top_k = edge_similarity_top_k or runtime.settings.edge_similarity_top_k
    min_strength = (
        edge_similarity_min_strength
        if edge_similarity_min_strength is not None
        else runtime.settings.edge_similarity_min_strength
    )
    ...
    async with runtime.session_factory() as session:
        try:
            result = await rebuild_knowledge_graph_edges_bulk(
                ...
                edge_similarity_top_k=top_k,
                edge_similarity_min_strength=min_strength,
                apply=apply,
            )
...
def _print_result(
    *,
    ...
    click.echo(f"top_k={result.edge_similarity_top_k}")
    click.echo(f"min_strength={result.edge_similarity_min_strength}")
...
@click.option(
    "--top-k",
    "edge_similarity_top_k",
    type=click.IntRange(min=1),
    default=None,
    help="Override KNOWLEDGE_API_EDGE_SIMILARITY_TOP_K.",
)
@click.option(
    "--min-strength",
    "edge_similarity_min_strength",
    ...
    default=None,
    help="Override KNOWLEDGE_API_EDGE_SIMILARITY_MIN_STRENGTH.",
)
def cli(
    *,
    apply_changes: bool,
