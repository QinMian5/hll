...
def _build_prompt(
    *,
    ...
    return (
        ...
        "Traverse taxonomy progressively and choose one taxonomy node.\n\n"
        "Executable and commands:\n"
        f"1) {tool_command} list-children [--parent-id <int>]\n"
        f"2) {tool_command} get-assignment --node-id <int>\n"
        f"3) {tool_command} assign-taxonomy-node --node-id <int> --taxonomy-node-id <int>\n\n"
        "Required execution workflow:\n"
        "- Start at root by listing children without parent-id.\n"
        "- Iteratively list children to descend the hierarchy.\n"
        "- Call assign-taxonomy-node exactly once for the selected taxonomy node.\n"
        "- If assignment returns already_assigned, keep that existing taxonomy node.\n\n"
