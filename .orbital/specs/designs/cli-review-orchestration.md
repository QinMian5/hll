---
abstract: Internal orchestration design for the CLI review workflow, covering the selected reviewer backend, typed review models, graph nodes, state, and dependency injection.
...
- **Scope/Boundaries:** Covers typed review schemas, reviewer-backend constraints, graph nodes, graph state, graph dependencies, and internal failure semantics. Excludes external CLI contract, end-user invocation examples, backend ingestion worker behavior, and direct database access.
...
- **Outputs:**
  ...
  - One shared Python function result used by both the CLI wrapper and other local orchestrators
- **Artifacts:**
  - `ReviewItem`
  - `ReviewResult`
  - `ReviewState`
...
## Design Approach
- **Approach:** Keep judgment inside one selected reviewer backend and keep orchestration inside a minimal graph. The graph exists only to make the internal state transition from review to submission explicit and testable.
...
- **Key Elements:**
  ...
  - **Shared submission entrypoint:** The app exposes one importable Python function that accepts `title` and `content`, executes the graph, returns `ReviewResult`, and raises on local runtime failures. The CLI command is only a wrapper over this function.
  - **Local failure serialization:** The CLI adds `hint` only after review completes. Hints are not generated by the reviewer backend; they are derived locally from the corresponding `ReviewResult` field descriptions and attached only to failed dimensions. Successful dimensions are omitted from stdout entirely, and success emits only the fixed English result marker `passed`.
  - **`ReviewItem`:** Contains:
    - `passed: bool`
    - `reason: str | None`
  ...
  - **Reviewer-backend rule:** The selected reviewer backend:
  ...
  - **Accepted reviewer backends:** First version supports:
    - a Cursor Agent backend that injects the `ReviewResult` JSON schema into the review prompt, validates returned JSON locally, and retries malformed responses up to the configured limit
    - an OpenAI-compatible backend selected behind the same reviewer callable boundary
  - **`ReviewState`:** Stores:
    - `title: str`
    - `content: str`
...
- **Checks:**
  ...
  - Reviewer-backend contract tests verify the selected backend output validates into `ReviewResult`.
  ...
  - Shared-entrypoint tests verify the importable Python function and the CLI wrapper project the same review/submission behavior.
- **Evidence:**
  - Passing tests for review models, reviewer-backend contract validation, graph branching, and dependency injection boundaries.
