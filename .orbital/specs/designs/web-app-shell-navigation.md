...
- **Key Elements:**
  ...
  - **Related-result item hover state:** Related-result items expose `Default`, `Hover`, and `Dimmed` visual states matching the approved Related Result Item Figma component set. Hover or keyboard focus strengthens the item border to the same blue emphasis used by Search result cards, raises the white surface opacity, swaps the default gray chevron for a `24px` blue circular right-arrow affordance, and keeps non-active sibling related items readable at approximately `80%` opacity.
...
- **Checks:**
  ...
  - Related-result items wrap long labels, grow each item row to fit the wrapped text, keep the right icon slot vertically centered, and expose hover/focus plus sibling-dim states aligned with the Search result card visual language.
