---
abstract: Shared frontend app shell design for the web client with top navigation, route ownership, and a Figma-aligned Search page.
...
## Context
- **Purpose:** Define the shared web app shell for the frontend so `Overview`, `Graph View`, and `Search` render inside one consistent top-level layout with route-driven navigation and a Figma-aligned Search page structure.
...
- **Detail Commitments:** The frontend uses one shared app shell for `Overview`, `Graph View`, and `Search`. The root route redirects to `Overview`. The shared top navigation displays `Overview`, `Graph View`, and `Search` in the center, with a left-side brand block and disabled `GitHub` and `Login` buttons on the right. `Search` follows Figma file `WBYs6P9HMxe21TSYQL637r`, node `128:90`, including an empty state and a results state within a single `/search` route. Shell styling is expressed primarily through Tailwind utility classes instead of page-owned handwritten CSS.
...
- **Inputs:**
  ...
  - Approved Figma reference for Search page composition: file `WBYs6P9HMxe21TSYQL637r`, node `128:90`.
...
- **Key Elements:**
  ...
  - **Top navigation:** The left side contains the blue square placeholder and `Knowledge Graph` brand label. The center contains `Overview`, `Graph View`, and `Search`. The right side contains disabled `GitHub` and `Login` buttons. Navigation highlight is route-driven: the active item uses darker text and a bottom underline, and inactive items use one consistent muted gray treatment.
