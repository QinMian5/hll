...
- **Key Elements:**
  ...
  - **Search results structure:** Desktop results use the approved Figma `ResultsContent / Responsive Grid` structure: one grid row, a left results track and right suggestions track split by a `3fr / 1fr` ratio, and a `28px` column gap. The right suggestions panel must be a grid track participant with fill sizing, not a fixed-width sidebar. The results card list uses a responsive grid with flexible columns that preserve the approved desktop card rhythm near the Figma reference width and reduce column count on narrower desktop widths instead of overflowing. Mobile results use a stacked content area with a single-column results grid and a suggestions section below the results list. Cards and suggestions inherit their radius, border, shadow, and typography from the approved Figma structures rather than from generic shared surface defaults.
