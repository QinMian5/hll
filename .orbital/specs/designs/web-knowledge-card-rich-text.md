---
abstract: Shared frontend rich-text rendering design for knowledge-card title, label, and content surfaces with Markdown support, inline LaTeX normalization, and KaTeX presentation.
...
## Context
- **Purpose:** Define one shared frontend rendering boundary for knowledge-card label, `title`, and `content` surfaces so every web surface that displays card text uses the same Markdown, inline LaTeX normalization, and KaTeX presentation behavior.
- **Scope/Boundaries:** Covers renderer ownership for knowledge-card text in `apps/web`, the accepted Markdown subset, `\(...\)` to `$...$` normalization, KaTeX stylesheet ownership, label/title/content rendering presets, and soft-failure behavior. Excludes backend content validation, data-model changes, search ranking semantics, and non-card rich-text surfaces.
...
- **Inputs:**
  ...
  - Existing web feature surfaces that display knowledge-card text, including Search results, taxonomy leaf title labels, taxonomy leaf hover disclosure, and taxonomy leaf selected disclosure.
...
- **Outputs:**
  ...
  - One controlled Markdown-plus-KaTeX rendering result for compact label surfaces.
...
## Design Approach
- **Approach:** Introduce one shared `KnowledgeRichText` rendering boundary in the web shared UI layer. The boundary accepts raw card text plus a presentation variant such as `label`, `title`, or `content`, normalizes inline LaTeX delimiters, renders through the Markdown math pipeline, and maps the resulting HTML structure onto approved card typography and spacing rules. Feature components continue to own their container layout, but they delegate all card text rendering to the shared renderer.
- **Key Elements:**
  ...
  - **Variant rule:** The renderer exposes presentation variants for the adopted card-text surfaces:
    - `label`: compact point-title label layout, centered text, tight line-height, muted or default text tone supplied by the host state, and no default block spacing that would create a card-like footprint.
  ...
  - **Feature-boundary rule:** Search result cards, taxonomy leaf title labels, taxonomy leaf hover disclosure, and taxonomy leaf selected disclosure keep ownership of layout, spacing, scrolling, and interaction state. They do not own Markdown or KaTeX parsing policy.
- **Interactions:**
...
- **Checks:**
  ...
  - Common Markdown constructs used by cards render correctly inside label/title/content constraints.
