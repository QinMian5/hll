...
- **Key Elements:**
  ...
  - **Interrupt handling:** On `Ctrl+C`, the coordinator records an `import-interrupted` event, stops accepting further results, and forcefully tears down all worker processes using a terminate-then-kill fallback so the import does not leave long-lived child processes behind. Shards already marked `completed` stay complete; shards still in `running` state remain resumable and are reclaimed on the next run.
