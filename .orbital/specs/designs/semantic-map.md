...
- **Key Elements:**
  ...
  - **Semantic level derivation rule:** At rebuild time, semantic levels are derived from assigned taxonomy leaves: include every occupied taxonomy depth band from root to deepest assigned leaf, then append one terminal card layer at `deepest_leaf_depth + 1`.
