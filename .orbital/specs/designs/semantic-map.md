...
- **Outputs:**
  ...
  - Published semantic-map snapshots produced by background build execution.
- **Artifacts:**
...
- **Key Elements:**
  - **Semantic-map module ownership:** `apps/api/src/modules/semantic_map` owns semantic-map read contracts, snapshot read orchestration, and snapshot build orchestration. It does not own graph persistence truth.
  - **Persistence projection source:** `apps/api/src/modules/semantic_map/persistence/model.py` defines the accepted snapshot persistence shape on shared SQLAlchemy metadata. Alembic revisions are derived from registered metadata through governed autogenerate flow; migration files are not the schema source of truth.
  ...
  - **Snapshot publication model:** Semantic-map artifacts are built in batch from persisted embeddings. Frontend browsing reads the latest successful snapshot through `manifest/current`, then pins subsequent tile reads to the returned snapshot `version`.
  - **Snapshot version format:** Phase 1 publishes snapshot `version` as `YYYYMMDD_HHMMSS_microseconds` so each build has one sortable external version token with a built-in anti-collision suffix.
  ...
  - **Build initiation surface:** Phase 1 build execution is initiated only through a dedicated operator command or script. No HTTP build endpoint is exposed, and ingestion does not auto-enqueue snapshot builds.
  - **High-level structure truth:** Semantic-map top-level structure comes from the persisted taxonomy tree rather than from embedding-only clustering. Semantic-map build consumes taxonomy nodes and final node-to-leaf assignments as the structural truth for region hierarchy.
  ...
  - **Region emission rule:** Build emits taxonomy regions only for non-empty taxonomy nodes (nodes with at least one assigned card in descendants). Empty taxonomy branches are excluded from all semantic-map tiles.
  ...
  - **Semantic level derivation rule:** At build time, semantic levels are derived from assigned taxonomy leaves: include every occupied taxonomy depth band from root to deepest assigned leaf, then append one terminal card layer at `deepest_leaf_depth + 1`.
  ...
  - **Phase 1 world normalization:** Build normalizes projected coordinates into one fixed Cartesian world extent `[0.0, 0.0, 1000.0, 1000.0]`, with `default_view.target = [500.0, 500.0]` and `default_view.zoom = 0.0`.
  - **Empty-source behavior:** When no projection nodes are available, build returns without publishing a new snapshot so the frontend continues to see the latest successful version or the explicit no-snapshot empty state.
  ...
  - **Manifest role:** Semantic-map manifest is a snapshot-self-contained read bootstrap document. It includes `semantic_levels` so read APIs do not depend on live taxonomy services for semantic-level resolution.
  ...
  - **Snapshot consistency:** A browsing session may keep reading one snapshot `version` while a newer snapshot is published. Build failures do not replace the current snapshot.
...
- **Interactions:**
  ...
  - Background semantic-map build execution reads persisted taxonomy truth through `taxonomy` service ports, reads persisted knowledge truth through `knowledge_graph` service ports, and publishes a new snapshot on success.
