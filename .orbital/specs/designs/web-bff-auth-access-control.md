---
abstract: Public web BFF, Logto session, anonymous identity, authenticated Search proposal submission, Workspace proposal review, quota, role enforcement, and private API access-control design for the web application.
out_of_scope: MCP server implementation, backend domain ranking semantics, Workspace apply internals, and Logto tenant provisioning runbooks.
---
...
## Context
- **Purpose:** Define the public web access boundary so browser users can use Graph View and Search anonymously with strict quotas, signed-in users can submit Search proposals and manage Dashboard tokens through the BFF, reviewers can access role-gated Workspace review actions through the BFF, and browser code cannot call private service APIs as public product interfaces.
- **Scope/Boundaries:** Covers the `apps/web` Express BFF runtime, browser-visible web API endpoints, Logto server-side session handling, authenticated Search proposal submission, Workspace proposal tracking/review actions, Knowledge-owned role enforcement at the web boundary, Logto Account API profile access for web account settings, Logto Management API orchestration for Dashboard tokens, anonymous identity cookies, Redis-backed quota state, private API calls, MCP internal usage summary calls, public route exposure, and first-version validation expectations. Excludes MCP public tool implementation, backend search ranking semantics, Workspace apply internals, and operational Logto tenant setup instructions.
...
- **Outputs:**
  ...
  - Browser-visible public repository summary endpoint for shell repository-link metadata.
  - Browser-visible web data endpoints for route-addressable Graph View, Search proposal submission, Workspace proposal tracking/review, and Dashboard token management.
...
- **Key Elements:**
  ...
  - **Static asset cache policy:** Production HTML application shell responses use `Cache-Control: no-cache`. Production hashed Vite assets under `/assets/*` use `Cache-Control: public, max-age=31536000, immutable`. Missing `/assets/*` paths return JSON `404` responses with `Cache-Control: no-store` and do not enter the application-shell fallback. Browser-side Vite preload errors call `preventDefault`, store a session-scoped reload marker, and reload the page at most once per browser session.
  ...
  - **Repository summary endpoint:** The BFF exposes `GET /web-api/repository-summary` for public repository-link metadata used by the shared shell. The endpoint requests GitHub repository metadata for the public repository, reads `stargazers_count`, returns the repository URL and star count as browser-safe JSON, caches successful metadata responses in process memory, and returns a safe unavailable error when GitHub metadata cannot be loaded.
  - **Web data endpoints:** The BFF exposes `GET /web-api/search` for the Search page, `GET /web-api/taxonomy/view/root` for Graph View root child-list state, `GET /web-api/taxonomy/view/path/{route_path}` for canonical LCC slug-path Graph View state with nested path capture, `GET /web-api/taxonomy/view/nodes/{node_id}` for id-addressed taxonomy view access, `GET /web-api/taxonomy/view/card-scopes/layout` for viewport-scoped card-scope layout slices, `POST /web-api/taxonomy/view/card-scopes/titles` for viewport-scoped title hydration, and `POST /web-api/taxonomy/view/card-scopes/details` for viewport-scoped card hydration.
  - **Proposal endpoints:** The BFF exposes authenticated web endpoints for creating proposals from Search, listing the current user's proposals for Workspace, listing reviewer-visible pending proposals, withdrawing pending self-submitted proposals, rejecting pending proposals, and accepting/applying pending proposals. The browser request body never contains user identity or role fields. The BFF supplies the authenticated Logto user id from the server-side session before calling the private API.
  - **Search proposal endpoints:** Search card proposal submission uses the unified proposal endpoints for `create`, `edit`, and `delete` proposals. Search remains the lightweight source of contributor-facing card maintenance proposals.
  ...
  - **Internal route mapping:** `GET /web-api/search` maps to private `GET /api/v1/search`. Proposal routes map to private proposal APIs through generated internal API client methods. `GET /web-api/taxonomy/view/root` maps to private `GET /api/v1/taxonomy/view/root`. `GET /web-api/taxonomy/view/path/{route_path}` maps to private `GET /api/v1/taxonomy/view/path/{route_path:path}`. `GET /web-api/taxonomy/view/nodes/{node_id}` maps to private `GET /api/v1/taxonomy/view/nodes/{node_id}`. `GET /web-api/taxonomy/view/card-scopes/layout` maps to private `GET /api/v1/taxonomy/view/card-scopes/layout`. `POST /web-api/taxonomy/view/card-scopes/titles` maps to private `POST /api/v1/taxonomy/view/card-scopes/titles`. `POST /web-api/taxonomy/view/card-scopes/details` maps to private `POST /api/v1/taxonomy/view/card-scopes/details`.
  ...
  - **Error responses:** Quota failures return `429` with `Retry-After` when available and a machine-readable web error code. Invalid or expired sessions are treated as anonymous when the endpoint permits anonymous use. Authenticated-only proposal submission returns `401` when no valid session is present. Reviewer/admin-only Workspace actions return authorization errors when the authenticated user lacks the required Knowledge-owned role. Internal API failures are mapped to web responses without exposing internal stack traces.
...
- **Interactions:**
  ...
  - The shared shell calls `/web-api/repository-summary` for repository-link star-count metadata. If the endpoint is unavailable, the shell keeps the repository link usable and displays the plain `GitHub` label.
  ...
  - Logged-in Search proposal requests resolve the server-side Logto session, attach the Logto user id as the proposal principal, and are forwarded by the BFF to the private proposal API.
  - Logged-in Workspace reviewer actions resolve the server-side Logto session and rely on Knowledge-owned role checks before private review/apply APIs perform state transitions.
...
- **Checks:**
  ...
  - Browser-side shell code calls `/web-api/repository-summary` for repository-link star-count metadata and handles unavailable metadata without breaking shell navigation.
  ...
  - BFF server tests cover authenticated Search proposal forwarding, unauthenticated proposal rejection, reviewer/admin role enforcement for Workspace actions, and absence of browser-supplied user identity or role fields in forwarded proposal payloads.
  ...
  - BFF application-shell tests cover production HTML cache headers, immutable hashed asset responses, and non-cacheable missing-asset `404` responses.
  - Browser preload-error tests cover controlled one-time reload behavior for Vite preload failures.
  - BFF internal API adapter tests verify generated client usage and error mapping for Search, proposal routes, and taxonomy view calls.
  ...
  - Frontend tests cover auth action rendering, anonymous state rendering, logged-in state rendering, quota error presentation, Workspace role-gated view access, and existing Graph View/Search data-loading behavior through web adapters.
