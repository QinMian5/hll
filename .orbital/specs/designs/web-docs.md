---
abstract: Web MCP setup route for client setup guidance across Codex, Claude Code, and OpenClaw.
...
## Context
- **Purpose:** Provide one first-class MCP setup route inside the shared web app shell so MCP client setup for HLL has a stable product-owned home.
...
- **Detail Commitments:** The web client exposes `/docs` as a routed page inside the shared app shell. `MCP Setup` is a primary navigation item alongside `Overview`, `Graph View`, and `Search`. The current `/docs` page is an MCP client setup surface titled `MCP Client Setup`, with a `Clients` selector and a selected-client `Configuration` panel. The first-version clients are `Codex`, `Claude Code`, and `OpenClaw`. Selecting one client shows only that client's setup steps. Dashboard remains the authenticated token and quota management surface. The `/docs` page owns canonical explanatory guidance for connecting external clients to the environment-specific HLL MCP public endpoint supplied by the Web BFF, and each client flow starts by directing the user to create or copy a Dashboard personal access token. The Web BFF owns `KNOWLEDGE_WEB_MCP_PUBLIC_BASE_URL` as a first-class public configuration value; development uses `http://localhost:8002/mcp`, and production uses `https://knowledge.orbitalis.org/mcp`.
- **Update Rule:** Repository-level requirements remain stable while MCP setup route ownership, information architecture, page content structure, and navigation placement stay in this design document.
...
- **Outputs:**
  - An MCP setup page rendered inside the shared app shell at `/docs`.
  - A visible primary navigation entry for `MCP Setup`.
...
## Design Approach
- **Approach:** MCP Setup is a product route, not an account submenu item. It uses the existing shared shell and restrained Knowledge visual language, with a static client selector plus selected-client setup panel before a markdown or search-backed docs system exists.
- **Key Elements:**
  - **Route ownership:** `/docs` is a top-level route lazy-loaded through the app router.
  - **Primary navigation:** The shared shell renders `MCP Setup` as an enabled primary navigation item linking to `/docs`, with the same row height, spacing, active state, and drawer behavior as the existing primary routes.
  ...
  - **Client setup taxonomy:** `Codex`, `Claude Code`, and `OpenClaw` are first-version client pages inside the selector. Each client starts with a Dashboard token instruction step rendered as a non-terminal guidance block without a copy control. Codex then shows four command/configuration steps: add the `hll` server, set bearer authentication in `~/.codex/config.toml`, inspect the saved server, and list configured servers. Claude Code then shows three command steps: add the `hll` HTTP server with an `Authorization` header, inspect the saved server, and list configured servers. OpenClaw then shows three command steps: save an `hll` Streamable HTTP server with an `Authorization` header, inspect the saved server, and list configured servers. Only the selected client's steps are visible.
  ...
  - **Dashboard boundary:** Dashboard remains the place to create, inspect, copy, rename, and delete personal access tokens. The `/docs` page remains the place to explain what those tokens are for and how external clients should be configured.
  - **Visual language:** The route follows the existing light product-shell style: Geist typography, neutral page background, white bordered panels, `8px` radii, restrained blue emphasis, Lucide copy icons, and product-client icons imported from Figma assets. The `/docs` page title sizing follows the shared routed-page title tokens rather than Docs-local display sizing. Docs-specific page background, panel heading sizes, copy controls, scrollbars, terminal chrome, client rows, and step badges are represented with theme tokens rather than ad hoc arbitrary utilities.
- **Interactions:**
  - Navigating to `MCP Setup` changes the route to `/docs` and highlights only the MCP Setup primary navigation item.
  - The Web BFF serves HTML with browser runtime configuration containing the current environment's MCP public endpoint.
  - On mobile, selecting `MCP Setup` from the drawer closes the drawer through the same route-navigation behavior as other primary routes.
  ...
  - MCP setup content is static in the first version and does not call backend APIs.

## Validation
- **Checks:**
  - App shell tests verify `MCP Setup` is present in desktop navigation and mobile drawer navigation.
...
- **Evidence:**
  ...
  - The primary navigation contains `Overview`, `Graph View`, `Search`, and `MCP Setup`.
