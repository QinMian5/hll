...
- **Key Elements:**
  ...
  - **Leaf performance-boundary rule:** Leaf camera movement is owned by a high-frequency viewport store that is decoupled from React component state. Dragging or zooming the deck scene may update this store every frame, but only viewport-slice requests, hydration, visible-title-set recomputation, and other data-side overlay work observe it through throttled or frame-bounded snapshots. Visible title labels and disclosure positions must stay camera-synchronous without requiring full-tree React rerenders on every pointer frame.
  ...
  - **Leaf viewport-slice rule:** The frontend converts the live deck viewport plus overscan into world bounds and requests leaf layout slices for those bounds from the backend.
  - **Leaf title hydration rule:** Point-title mode is driven by viewport-scoped title hydration. The frontend requests `title` for node ids returned by the current layout slice and inside the current viewport plus one overscan ring, then uses the title for the label overlay.
  ...
  - **Zoom activation rule:** Point-title hydration and point interaction are gated by one deck orthographic viewport zoom threshold: `0.85`. Below `0.85`, all leaf nodes remain in non-interactive point mode. At or above `0.85`, point-title mode is active and point glyphs can be hovered or clicked. Activation and deactivation use the same threshold and are driven by the live deck viewport frame instead of the throttled viewport snapshot.
...
- **Checks:**
  ...
  - Hover and selected disclosures share one visual card shell, use the tokenized `8px` disclosure radius, use the accepted `md`, `lg`, `xl`, and `2xl` maximum card size family, shrink below the maximum height when content is short, keep long titles horizontally scrollable in the header, and keep overflow content inside the disclosure content scroll region.
  - Dragging or zooming the leaf scene does not trigger full-overlay-set synchronous DOM measurement, disclosure position React state updates, fixed screen-edge clamps, or repeated markdown/KaTeX parsing for unchanged text.
