...
## Design Approach
- **Approach:** Use two dedicated client-owned layout pipelines inside the shared React Flow canvas. Branch layout prioritizes weighted category browsing and visual breathing room over explicit edge rendering. Leaf layout prioritizes relation readability and interaction performance through level-of-detail rendering: skeleton points plus true edges by default, viewport-scoped card hydration only after zoom activation, and node content available through hover instead of default canvas occupancy. Branch and leaf therefore use separate visual systems that match their approved Figma references while preserving one stable graph coordinate space.
- **Key Elements:**
  ...
  - **Leaf anchor rule:** The card center is anchored to the original point coordinate. Hydrating a node changes its rendered footprint but does not move its graph anchor.
  - **Leaf card sizing rule:** Card width uses a fixed maximum width and a stable minimum width. Text wraps inside that width and card height grows with the measured number of title lines.
  ...
  - **Leaf rectangle collision rule:** When nodes are upgraded into cards, the layout layer must use the card's true rectangular width and height for visibility checks and collision handling rather than the old circular bubble diameter.
...
- **Interactions:**
  ...
  - Entering a leaf view computes the one-hop layout from the returned skeleton payload and renders all nodes in point mode with the true graph edges still visible.
  - Crossing the card-activation zoom threshold computes the active viewport plus overscan set and hydrates leaf node details only for that region.
  - Panning inside card mode incrementally hydrates newly entered overscan nodes while leaving far-away nodes in point mode.
...
- **Checks:**
  ...
  - Hydrated leaf cards are center-anchored to the original point coordinates, use fixed maximum width with automatic line wrapping, and grow in height with line count.
