...
- **Scope/Boundaries:** Covers branch/root bubble sizing and placement, leaf graph level-of-detail behavior, Figma-aligned branch bubble composition, hover and selected content disclosure behavior, typography, Tailwind-first styling boundaries, viewport-driven title hydration, and layout-stability rules for `apps/web`. Excludes taxonomy payload ownership, page-shell header and overlay structure, and backend graph semantics beyond the consumed skeleton/detail contracts.
...
- **Inputs:**
  ...
  - Leaf content detail batches containing `title` and `content` for interacted point ids in point-title mode.
  ...
  - Approved Figma references for leaf point mode, point-title mode, selected disclosure, and no-selection states in file `WBYs6P9HMxe21TSYQL637r`: component nodes `818:353`, `818:360`, and `844:531`; desktop content nodes `799:743`, `808:303`, `834:1469`, and `834:2492`; mobile content nodes `799:744`, `808:441`, `834:2005`, and `834:3028`.
...
- **Outputs:**
  ...
  - One leaf point, point-title, edge, hover-disclosure, selected-disclosure, and no-selection presentation system aligned with Figma file `WBYs6P9HMxe21TSYQL637r`.
...
- **Key Elements:**
  ...
  - **Leaf on-demand content rule:** Hovering or selecting a point glyph in point-title mode requests that node's disclosure detail when it is missing from cache. Disclosure detail contains `title` and `content`; cached title data may be reused while content remains interaction-driven.
  ...
  - **Leaf hover state rule:** When no node is selected, hovering a point glyph in point-title mode makes the hovered node the transient focus source and displays a content-only hover disclosure after disclosure detail is available.
  ...
  - **Leaf disclosure rule:** Leaf `content` is revealed through a lightweight floating disclosure that visually belongs to the canvas. Hover disclosures are transient and content-only. Selected disclosures are persistent, include the selected node title plus content, stay centered to the selected point, and sit near the selected point.
  ...
  - **Leaf selected-title rule:** In point-title mode, the selected node hides its title label only while the selected title-plus-content disclosure is visible. The selected title moves into the selected disclosure. When selected disclosure disappears, the node's title label is restored if the node remains in the visible title set. Hover disclosure does not hide the title label.
  - **Leaf label text rule:** Point-title labels, selected disclosure titles, hover disclosure content, and selected disclosure content consume the shared knowledge-card rich-text contract defined in `web-knowledge-card-rich-text.md`.
  ...
  - **Selected disclosure placement rule:** Selected disclosure anchors to the selected node coordinate, remains horizontally centered to that point, and uses the Figma selected disclosure viewport width for desktop and mobile.
...
- **Checks:**
  ...
  - Point-title labels, hover disclosure content, selected disclosure title, and selected disclosure content use the shared knowledge-card rich-text renderer instead of raw string rendering.
